...
// carry real timetables. Ankara has no open feed and comes from OpenStreetMap; İzmir has its
// own portal. Neither publishes bus TIMES, so their schedule files are deliberately empty
// rather than filled with invented headways.
// İzmir came from OSM at first and was rejected — 29 routes at ~6 stops each. It now comes
// from İzmir Büyükşehir's own open-data portal instead: 441 ESHOT lines, 71 stops per
// direction, plus road geometry, which is better than the OSM-sourced Ankara set. See
// fetch-izmir-bus.cjs. Bursa and Antalya remain excluded: OSM holds 4 and 3 usable routes
// for them, and neither city's portal was reachable to check for something better.
...
const BUS_CITIES = {
  ...
  izmir:    { dir:'izmir-bus-directory.json', graph:'izmir-bus-graph.json', sched:'izmir-bus-schedules.json', geom:'izmir-bus-geom.json' }
};
